   ...
   public  class UserInfo
    {
        ...
        public IEnumerable<Model.UserInfoNo> GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over by username) as num,userinfo.* from userinfo";
                sql += "select * from a where num between (@pageIndex-1)*@pageSize+1 and @pageSize+1 and @pageIndex*@pageSize;";
